  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        login('user', token, userData, refreshToken);
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark flex items-center justify-center p-6 geo-pattern hero-gradient">
      <div className="glass-card w-full max-w-md rounded-[2.5rem] p-10 border border-white/50 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          <h1 className="text-3xl font-black text-primary dark:text-white tracking-tight uppercase">User Login</h1>
          <p className="text-sm text-gray-500 font-bold mt-2 uppercase tracking-widest">Secure Gateway Access</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 ml-4" htmlFor="username">Username Identifer</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 ml-4" htmlFor="password">Access Token</label>
            <div className="relative group">
        ...
        <div className="mt-8 pt-8 border-t border-gray-100 flex flex-col items-center gap-4 text-center">
          <p className="text-sm font-bold text-gray-500">
            Internal record needed? <Link to="/user-register" className="text-electric-blue hover:underline">Create Account</Link>
          </p>
          <p className="text-xs font-black uppercase tracking-widest text-gray-300">
            Legal Professional? <Link to="/lawyer-login" className="text-primary hover:underline">Access Authority Portal</Link>
          </p>
        </div>
      </div>
    </div>
